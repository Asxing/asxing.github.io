---
title: SprngBoot-Redis启动加载预置数据
author: HoldDie
tags: [SpringBoot,Spring,Redis]
top: false
...
> 最锋利的武器，抵不过一颗无惧的心。 ——天沧苍

### 需求

通常数据库中有一些数据时亘古不变的，并且使用特别频繁，首先就想到了当程序启动的时候，将高频数据放入到Redis中，由于项目原因，此时列举 Spring 方式 和 SpringBoot 方式。
